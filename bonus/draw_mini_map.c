...
/*   Updated: 2025/09/11 17:19:45 by wkannouf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_border(t_data *data, int demi_rayon)
{
	...
	while (angle < 360)
	{
		...
		data->donnee->draw_x = center_x + demi_rayon * cos(rad);
		data->donnee->draw_y = center_y + demi_rayon * sin(rad);
		draw_square(data->game->img, data->donnee->draw_x,
			data->donnee->draw_y, 0x3e3e42FF);
		angle++;
	}
}
...
static void	draw_walls_doors_vide(t_data *data, int demi_rayon, int x, int y)
{
	...
	{
		data->donnee->draw_x = center_x + (res_x * 7.5);
		data->donnee->draw_y = center_y + (res_y * 7.5);
		if (data->map_data->map[y][x] == '1')
			draw_square(data->game->img, data->donnee->draw_x,
				data->donnee->draw_y, 0xD42525FF);
		else if (data->map_data->map[y][x] == 'D')
			draw_square(data->game->img, data->donnee->draw_x,
				data->donnee->draw_y, 0x000000FF);
		else
			draw_square(data->game->img, data->donnee->draw_x,
				data->donnee->draw_y, 0x696969FF);
	}
}
